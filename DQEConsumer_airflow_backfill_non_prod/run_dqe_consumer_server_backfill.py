...
if __name__ == "__main__" :
    ...
    localport = 9003 ## SK: NOTE, check your port number here: if connecting database locally, change port number here
    ...
    backfill_data_path = "./data/2023_tds_full_apr25.csv"
    ...
    if test_mode == 0:
        ...
        #"""SK: NOTE: hardcode time frame here LINE 104 and LINE 105"""
        # sql_st1 = """select distinct tpt_id_key , create_date from dqe_consumer.insert_tpt_id 
        #             where create_date >= '2021-11-17 00:00:00' 
        #             and create_date <= '2021-11-17 16:00:00' """
      ...
      #              where document_type.code_value in ('PROT') and 
      #              field_testing_type in (2) and
      #              status.code_value in ('1') AND 
      #              ft.field_year in ({}) 
      #              ORDER BY TPT_ID_KEY """.format(test_year)
        ...
        sql_st1 = """ Select DISTINCT 
                        ...
                        document_type.code_value
                            FROM scout_internal.field_testing ft
                     JOIN scout_internal.master_code document_type ON document_type.code_id = ft.field_testing_type_code_id
                     JOIN scout_internal.master_code category ON category.code_id = ft.category_code_id
                     JOIN scout_internal.master_code study ON study.code_id = ft.study_code_id
                         JOIN scout_internal.master_code status ON status.code_id = ft.status_code_id
                         JOIN scout_internal.trial_responsible tr ON tr.field_testing_id = ft.field_testing_id
                         LEFT JOIN quality_dashboard.region region ON region.test_location = substring(replace(tr.site_name,'NAR - ', ''), 1, 3)
                         LEFT JOIN scout_internal.field_responsible fr on fr.field_testing_id = ft.field_testing_id
                         LEFT join scout_internal.master_code responsible_type on responsible_type.code_id = fr.responsible_type_code_id
                         where document_type.code_value in ('PROT') and 
                         ft.field_year in ({}) and field_testing_type in (2) and status.code_value in ('1') and
                         category.code_value in ('A','D','R') and study.code_value in ('I','F','S','H','G') 
                         /*and ft.tpt_id_key in ('HD23NZLAA1')*/ /*and responsible_type.decode1 in ('Licence User') */ and responsible_type.code_value in ('2')
                         order by ft.tpt_id_key""".format(test_year) 
        ...
        tpt_ids_dict = {row[0]: {'tpt_id_key': row[0], 'doc_type': row[2], 'year': int(row[1])} for row in cursor.fetchall() }
        ...
        #     # where tpt_id_key in ('IA22WLDV2NGX')""" #.format(tuple(tpt_id_key_list))
        # print("sql_st2:\n", sql_st2)
    ...
    #################################
    counter = 0
    print(tpt_id_key_list[startind:endind])
    for tpt_id_key in tpt_id_key_list[startind:endind]:
        ...
        if doc_type == 'TD' and len(tpt_id_key) == 12:
            ...
            if td_0_data and 'trialDescriptions' in td_0_data['data'] and td_0_data['data']['trialDescriptions']:
                ...
                input_dqe['tests'] = [ 
                            { "TDCompleteness_0": {
                                "attributeMapping": "/home/ubuntu/DataQualityEngine/tests/Files/attribute_mapping_TD_0.py",
                                "weights": "/home/ubuntu/DataQualityEngine/tests/Files/completeness_v2.csv",
                                "version": "completeness_TD_V2_0_0" 
                                }
                            }
                ]
        ...
        ## get protocol result
        elif doc_type == 'PROT' and len(tpt_id_key) == 10:
            doc_type = 'Protocol'
            ...
            if protocol_data and 'protocols' in protocol_data['data'] and protocol_data['data']['protocols']:
                ...
                ## pass the td1 result to the total result
                if protocol_dqe_result and len(protocol_dqe_result[0]['ProtocolCompleteness']) > 0:
    ...
    if exe_env == 'local' or exe_env == 'skip' and saves3flag == 0:
        ...
        subfolder = 'logs/backfill_airflow/run_' + datetime.datetime.utcnow().strftime(out_dateformat)
